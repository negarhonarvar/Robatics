...
# in this code we Consider right wheel velocity to be equal to sin(t)
...
# General parameters
max_speed_left = np.cos(robot.getTime()) * (-1)  # Angular speed in rad/s
max_speed_right = np.sin(robot.getTime()) * (1)
...
while robot.step(TIME_STEP) != -1:
    left_motor.setVelocity(np.cos(robot.getTime()) * (-1))
    right_motor.setVelocity(np.sin(robot.getTime()) * (1))
